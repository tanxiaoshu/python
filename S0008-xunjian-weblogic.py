...
def port():
	...
	if Pid['value'] == 0:
		...
		cmd = "netstat -lntp|grep %s|head -1" % Pid['value']
		port = subprocess.Popen(cmd,shell=True,stdout=subprocess.PIPE).stdout.read().strip()
		p = re.search("^tcp.*:(\d+)",port).group(1)
		return {'port':{'value':p,'unit':0,'status':0}}
